...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_cups_league_id') THEN
    CREATE INDEX idx_cups_league_id ON cups(league_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_cups_status') THEN
    CREATE INDEX idx_cups_status ON cups(status);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_cup_groups_cup_id') THEN
    CREATE INDEX idx_cup_groups_cup_id ON cup_groups(cup_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_cup_teams_cup_id') THEN
    CREATE INDEX idx_cup_teams_cup_id ON cup_teams(cup_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_cup_teams_group_id') THEN
    CREATE INDEX idx_cup_teams_group_id ON cup_teams(group_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_cup_matches_cup_id') THEN
    CREATE INDEX idx_cup_matches_cup_id ON cup_matches(cup_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_cup_matches_group_id') THEN
    CREATE INDEX idx_cup_matches_group_id ON cup_matches(group_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_profiles_managed_league_id') THEN
...
DROP POLICY IF EXISTS "Anyone can view cup groups" ON cup_groups;
DROP POLICY IF EXISTS "Admins can manage cup groups" ON cup_groups;

DROP POLICY IF EXISTS "Anyone can view cup teams" ON cup_teams;
DROP POLICY IF EXISTS "Admins can manage cup teams" ON cup_teams;

DROP POLICY IF EXISTS "Anyone can view cup matches" ON cup_matches;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_cups_updated_at'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_cup_matches_updated_at'
  ) THEN
